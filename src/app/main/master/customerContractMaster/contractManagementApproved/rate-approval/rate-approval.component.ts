...
export class RateApprovalComponent implements OnInit {
  ...
  formValidation() {
    ...
    this.seachCrit = this.formbuilder.group({
      ...
      laneId: [null],
      matrixTypeId: [null],
      laneType: ["", Validators.required],
      action: ["", Validators.required],
      name: [''],
    })
    let sessionData: any = sessionStorage.getItem('contractIdBackUpForRateApproval')
    if (sessionData) {
      debugger
  ...
  submitRate() {
    ...
    sessionStorage.setItem('filterSearchfrRApp', JSON.stringify(this.seachCrit.value))
    this.route.navigate(['/master/rate-approval/rate-approval-list'], this.openPopJson.contractId)
    sessionStorage.removeItem('contractIdBackUpForRateApproval')
  ...
  laneType = [
    { id: 1, type: 'All' },
    { id: 2, type: 'Active Lanes' },
    { id: 3, type: 'Inactive Lanes' },
  ]
  activeType = [
    { id: 1, type: 'Approve Lanes' },
    { id: 2, type: 'Inactive Lanes' },
  ]
