...
@Component({
  ...
  templateUrl: './complaint-fwd.component.html',
  styleUrls: ['./complaint-fwd.component.scss']
})
export class ComplaintForwardComponent implements OnInit {
  ...
  itemDetailsById:any=""
  ngOnInit(): void {
    this.formComplaintmaster = this.formbuilder.group({
     ...
     let itemMasterId = res.data
      if (itemMasterId) {
        this.stryservice.getAllItemListDetailsById(itemMasterId).subscribe((res: any) => {
          ...
          this.formComplaintmaster.patchValue(res.data)
        })
      }
    })
  }
  ...
  get f() {
    return this.formComplaintmaster.controls;
  }

  ItemSubmit(){
    this.submitted = true;
    if (this.formComplaintmaster.invalid) {
      return
    }
    var itemJson = new itemMaster
    itemJson = this.formComplaintmaster.value
        ...
        if (err instanceof HttpErrorResponse) {
          if (err.status === 422) {
            ...
            Object.keys(err.error.Errors).forEach((prop: any) => {
              const formControl = this.formComplaintmaster.get(err.error.Errors[prop]?.PropertyName);
              //wrong key comes 
              if (formControl) {
